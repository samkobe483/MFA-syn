...
def save_best_model(state_dict, model_dir: str, best_epoch: int, keep: int):
    """
    Save the current best model, keep the most recent `keep` model checkpoints, and delete the remaining old models.

    Args:
    state_dict: current model parameter dictionary
    model_dir: model save directory
    best_epoch: current epoch number
    keep: the maximum number of best models to be retained
    """
...
def find_best_model(model_dir: str):
    """Find the model weights file in the current model directory and point to the full path of the best model"""
    model_files = [f for f in os.listdir(model_dir) if os.path.splitext(f)[-1] == '.pkl']
    epochs = [int(os.path.splitext(f)[0]) for f in model_files if str.isdigit(f[0])]
